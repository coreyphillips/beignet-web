...
import { BrowserWalletSession, type VaultStatus, type WalletDefaults } from '@/lib/local/client';
...
export function LocalConnection({ onConnect, back, BackupView }: {
  ...
  async function openWallet(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      if (!opened) {
        ...
        const result = await session.current.open(status.passwordRequired || addPassword ? password : '');
  ...
  return <section className="surface connection-card flow-stack">
    ...
    {wallets.length ? <div className="flow-stack">
      ...
      {!status?.exists && <p className="muted">Next, save your recovery phrase. Connections are configured automatically.</p>}
