...
Chrome (152 at the time of writing) treats a public site reaching a loopback or private address as local network access and holds the connection until the user allows its permission prompt; the wallet's sockets stay pending until then. From the HTTPS site only loopback addresses may be plain `ws://`; anything else needs `wss://` with a certificate the browser trusts, and the primary must accept WebSocket connections from the site's origin. The primary URI keeps its `pubkey@host:port` form; the WebSocket URL is a separate field. A browser tab is still not an always-on node.
